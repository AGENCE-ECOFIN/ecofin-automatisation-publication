...
import { FaArrowRight, FaArrowLeft, FaRss, FaGlobe, FaClock, FaUsers, FaFacebook, FaLinkedin, FaTwitter, FaTimes, FaCheck, FaSave } from 'react-icons/fa';
import { api } from '../services/api';

const FeedEditModal = ({ isOpen, onClose, feed, onSave, isLoading }) => {
  ...
  useEffect(() => {
    if (isOpen && feed) {
      ...
      setFormData({
        name: feed.name || '',
        url: feed.url || '',
        frequency_minutes: feed.frequency_minutes || 120,
        target_networks: feed.target_networks || [],
        social_pages: feed.social_pages || {},
  ...
  const handleInputChange = (e) => {
    ...
    if (name.startsWith('network_prompts.') || name.startsWith('social_pages.')) {
  ...
  const handleSubmit = async () => {
    ...
    console.log('📤 Mise à jour du flux:', formData);
    await onSave(feed.id, formData);
    handleClose();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <label className="text-sm font-medium text-gray-700">
          Flux actif (collecte automatique)
        </label>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col my-8">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold">Modifier le flux RSS</h2>
